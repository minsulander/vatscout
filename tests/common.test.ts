...
describe("common functions", () => {
    ...
    test("extract runway in use EDDF", () => {
        const riu = extractRunwayInUseFromAtisText("FRANKFURT DEPARTURE INFORMATION D MET REPORT TIME 1020 .. RUNWAYS IN USE 07C AND 18 .. ACFT DEPARTING RWY 18 EXPECT POSSIBLE DEPARTURE FROM INTERSECTION MIKE, TORA 3454 METERS, IF UNABLE ADVISE TWR ON INITIAL CONTACT .. TRL 60 .. WIND 070 DEGREES 6 KNOTS VARIABLE BETWEEN 030 AND 120 DEGREES .. CAVOK .. TEMPERATURE 25 DEW POINT 10 .. QNH 1022 .. TREND NOSIG .. FRANKFURT INFORMATION D OUT")
        expect(riu).toBe("07C/18")
    })

    test("extract runway in use KFJK", () => {
        const riu = extractRunwayInUseFromAtisText("KJFK ATIS INFO A 0951Z. 06005KT 10SM FEW250 20/18 A3010 (THREE ZERO ONE ZERO). APCH IN USE ILS RY 4R. DEPTG RY 4L.. NOTAMS... OPERATE XPDR ON MODE *C ON ALL TAXIWAYS AND RUNWAYS. READBACK ALL RWY HOLD SHORT INSTRUCTIONS.....ADVS YOU HAVE INFO A.")
        expect(riu).toBe("4R")
    })

    test("extract runway in use KISP", () => {
        const riu = extractRunwayInUseFromAtisText("KISP ATIS INFORMATION B. 0956Z. 00000KT 5SM BR FEW250 18/17 A3010 (THREE ZERO ONE ZERO). ILS RWY 24 APCH IN USE. LDG AND DEPTG RY 24. VISUAL APCH AVAILABLE ON REQUEST......ADVS YOU HAVE INFO B.")
        expect(riu).toBe("24")
    })

    test("extract runway in use EKCH", () => {
        let riu = extractRunwayInUseFromAtisText("THIS IS COPENHAGEN ARRIVAL INFORMATION C. AT 1020Z. EXPECT ILS APPROACH. RUNWAY IN USE FOR LANDING 22 LEFT. EXPECT CHARLIE STARS. VISUAL APPROACH ON REQUEST. TRANSITION LEVEL 60. 17007KT 9999 FEW027 20/14 Q1025 (ONE ZERO TWO FIVE)... AT FIRST CONTACT WITH APPROACH, STATE AIRCRAFT TYPE. THIS WAS COPENHAGEN ARRIVAL INFO C....ADVS YOU HAVE INFO C")
        expect(riu).toBe("22L")
        riu = extractRunwayInUseFromAtisText("THIS IS COPENHAGEN DEPARTURE INFO C. AT 1020Z. RUNWAY IN USE FOR TAKEOFF 22 RIGHT. 17007KT 9999 FEW027 20/14 Q1025 (ONE ZERO TWO FIVE).. AT FIRST CONTACT WITH APRON CONTROL ADVISE IF UNABLE TO PERFORM CUSTOMIZED PUSHBACK.. THIS WAS COPENHAGEN DEPARTURE INFO C....ADVS YOU HAVE INFO C")
        expect(riu).toBe("22R")
    })

    test("extract runway in use EBLG", () => {
        const riu = extractRunwayInUseFromAtisText("LIEGE INFORMATION C 1020 RNAV TRANSITION TO ILS APPROACH RUNWAY 22L FOR ARRIVALS AND DEPARTURES RUNWAY SURFACE CONDITION AT 1044 CLEAR AND DRY TRANSITION LEVEL 55 WIND 160 DEGREES 6 KNOTS CAVOK TEMPERATURE 23 DEW POINT 12 QNH 1022 NOSIG CONFIRM INFORMATION C ON FIRST CONTACT")
        expect(riu).toBe("22L")
    })

    test("extract runway in use ETOU", () => {
        const riu = extractRunwayInUseFromAtisText("WIESBADEN INFORMATION C MET REPORT TIME 0955 AUTOMATIC WEATHER REPORT EXPECT ILS APPROACH RUNWAYS IN USE 07 TRL 60120.805 WIND 060 DEGREES 5 KNOTS VISIBILITY MORE THAN 10 KILOMETERS CLEAR TEMPERATURE 24 DEW POINT 8 QNH 1022 CONDITION INFORMATION C OUT")
        expect(riu).toBe("07")
    })

    test("extract runway in use LGAV", () => {
        const riu = extractRunwayInUseFromAtisText("HERE IS ATHINA VENIZELOS AIRPORT ATIS INFORMATION C RECORDED AT 1420Z. APP TYPE ILS X. LDG RWY 03R. TAKE OFF RWY 03L. TRANSITION LEVEL 105. WIND 040 DEGREES 13G24KT WIND VARIABLE BETWEEN 010 AND 070 DEGREES VISIBILITY 10KM CLOUDS FEW 030 TEMPERATURE 30 DEW POINT 17 QNH 1012. SPECIFIC ATIS INSTRUCTION CONCENTRATION OF BIRDS IN THE AIRPORT VICINITY. ADVISE ON INITIAL CONTACT YOU HAVE LISTENED TO INFORMATION C.")
        expect(riu).toBe("03R")
    })
